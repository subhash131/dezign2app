              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="flex flex-col gap-1">
                  ...
                  <BufferedInput
                    ...
                    value={isAutoGeneratedStepName(step.name) ? "" : (step.name ?? "")}
                    onCommit={(val) => {
                      onChange({ ...step, name: val });
                    }}
                    debounceMs={350}
                    placeholder={
                      step.type === "condition"
                    ...
                    <BufferedInput
                      ...
                      onCommit={(val) => {
                        onChange({
                          ...step,
                          outputVariable: val,
                          name: isAutoGeneratedStepName(step.name) ? val : step.name,
                        });
                      }}
                      debounceMs={350}
...
export const StepRow = React.memo(StepRowComponent, (prev, next) => {
  if (prev.step !== next.step) return false;
  if (prev.index !== next.index) return false;
  if (prev.isFirst !== next.isFirst) return false;
  if (prev.isLast !== next.isLast) return false;
  if (prev.depth !== next.depth) return false;
  if (prev.serviceNodeId !== next.serviceNodeId) return false;
  if (prev.endpoint !== next.endpoint) return false;
  if (prev.consumedEvent !== next.consumedEvent) return false;
  if (prev.allNodes !== next.allNodes) return false;
  if (prev.allEdges !== next.allEdges) return false;
  if (prev.extraSources !== next.extraSources) return false;

  if (prev.priorSteps.length !== next.priorSteps.length) return false;
  for (let idx = 0; idx < prev.priorSteps.length; idx++) {
    if (prev.priorSteps[idx] !== next.priorSteps[idx]) return false;
  }
