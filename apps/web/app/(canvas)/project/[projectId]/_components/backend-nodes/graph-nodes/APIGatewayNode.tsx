  ...
  return (
    <div className={cn("shadow-md rounded-xl bg-card border-2 min-w-[280px] max-w-[380px] flex flex-col", selected ? "border-primary" : "border-border")}>
      ...
      <div className="px-3 py-2 border-b nodrag flex items-center gap-2">
        <Label className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider shrink-0">Auth</Label>
        <Select
          value={data.authType || "None"}
          onValueChange={(v) => updateNode(id, { data: { ...data, authType: v } })}
        >
          <SelectTrigger className="h-6 text-xs flex-1"><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="None" className="text-xs">None</SelectItem>
            <SelectItem value="JWT" className="text-xs">JWT</SelectItem>
            <SelectItem value="API Key" className="text-xs">API Key</SelectItem>
            <SelectItem value="OAuth2" className="text-xs">OAuth2</SelectItem>
            <SelectItem value="mTLS" className="text-xs">mTLS</SelectItem>
          </SelectContent>
        </Select>
      </div>
        ...
        {advancedOpen && (
          <div className="flex flex-col gap-2.5 pt-2 border-t border-border/50 nodrag">
            <div className="flex items-center justify-between gap-2">
              ...
              >
                <SelectTrigger className="h-6 text-xs w-[160px]"><SelectValue placeholder="Select..." /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="AWS API Gateway" className="text-xs">AWS API Gateway</SelectItem>
                  <SelectItem value="Kong" className="text-xs">Kong</SelectItem>
                  <SelectItem value="Nginx" className="text-xs">Nginx</SelectItem>
                  <SelectItem value="Traefik" className="text-xs">Traefik</SelectItem>
